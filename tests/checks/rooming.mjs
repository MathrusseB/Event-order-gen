...
/** Three nights in the Lodge's lower suites, and a party in the Bunk Room. */
function event() {
  return {
    meta: {
...
export async function run({ check }) {
  ...
  check(
    ...
    shape(middleOut).filter((row) => row.startsWith('Timber')).join(' | ')
      === 'Timber [a-dana] 11-13..11-14 | Timber [a-dana] 11-15..11-16',
    shape(middleOut).join('\n')
  );

  const elsewhere = placeGuest(middleOut, {
    guestId: 'a-dana', ...REMINGTON_1, night: N2,
  ...
  check(
    ...
    shape(elsewhere).filter((row) => row.includes('a-dana')).length === 3
      && shape(elsewhere).includes('1 [a-dana] 11-14..11-15'),
  ...
  check(
    ...
    shape(throughStay.event).filter((row) => row.startsWith('Wetland')).join(' | ')
      === 'Wetland [a-dana] 11-13..11-16' && throughStay.nights.length === 3,
  ...
  const joined = placeGuest(start, {
    guestId: 'a-tom', ...TIMBER, night: N2, how: 'add', through: false
  }).event;
  check(
    ...
    shape(joined).filter((row) => row.startsWith('Timber')).join(' | ')
      === 'Timber [a-dana+a-tom] 11-14..11-15 | Timber [a-dana] 11-13..11-14 '
        + '| Timber [a-dana] 11-15..11-16',
    shape(joined).join('\n')
  );
  ...
  const swapped = placeGuest(start, {
    guestId: 'a-tom', ...TIMBER, night: N2, how: 'swap', through: false
  }).event;
  check(
    'a swap exchanges the two rooms for that night and no other',
    shape(swapped).includes('Timber [a-tom] 11-14..11-15')
  ...
  const replaced = placeGuest(start, {
    guestId: 'a-tom', ...TIMBER, night: N2,
    how: 'replace', through: false
  }).event;
  check(
    'a replace takes the room and leaves the other guest in no room that night',
    shape(replaced).includes('Timber [a-tom] 11-14..11-15')
      && !shape(replaced).some((row) => row.includes('a-dana') && row.includes('11-14..11-15'))
      && replaced.attendees.length === 3,
    shape(replaced).join('\n')
  );

  // [v9] RLI is `named` now — it was pooled until v9, and a pooled building
  // wrote `room: null`. Every building takes a room number, because staff need
  // to know which room to service.
  const overflow = placeGuest(start, {
    guestId: 'a-dana', building: 'RLI', room: '8', night: N2, how: 'move', through: false
  }).event;
  check(
    'RLI takes a room number like every other building',
    overflow.rooming.some((row) => row.building === 'RLI' && row.room === '8'
